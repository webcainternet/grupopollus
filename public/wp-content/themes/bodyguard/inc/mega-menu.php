...
if ( !function_exists( 'wd_add_mega_menu_support' ) ):

	function wd_add_mega_menu_support() {
			...
			add_filter( 'wp_setup_nav_menu_item', 'wd_add_custom_nav_fields' );

			add_action( 'wp_update_nav_menu_item', 'wd_update_custom_nav_fields', 10, 3 );

			add_filter( 'wp_edit_nav_menu_walker', 'wd_edit_menu_walker', 10, 2 );

			//add_filter( 'nav_menu_css_class', 'wd_add_class_to_menu', 10, 2 );
...
add_action( 'init', 'wd_add_mega_menu_support' );



/* Add custom fields to menu */

if ( !function_exists( 'wd_add_custom_nav_fields' ) ):

	function wd_add_custom_nav_fields( $menu_item ) {

		
		$menu_item->mega_menu_icon = get_post_meta( $menu_item->ID, '_wd_mega_menu_icon', true )? get_post_meta( $menu_item->ID, '_wd_mega_menu_icon', true ) : '';
		
		$menu_item->mega_menu_bg_image = get_post_meta( $menu_item->ID, '_wd_mega_menu_bg_image', true )? get_post_meta( $menu_item->ID, '_wd_mega_menu_bg_image', true ) : '';
		
		$menu_item->mega_menu = get_post_meta( $menu_item->ID, '_wd_mega_menu', true ) ? 1 : 0;
...
if ( !function_exists( 'wd_edit_menu_walker' ) ):

	function wd_edit_menu_walker( $walker, $menu_id ) {



		class wd_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
				...
				$mega_menu_html = '';
  				...
  				$icons = unserialize( $icons );
		        	...
		        	$mega_menu_html .= '</select>
		                ...
		                '.__( 'display icone', 'bodyguard' ).'</label>
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                '.__( 'Mega Menu (Background image)', 'bodyguard' ).'
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
		                ...
		                '.__( 'Mega Menu (classic)', 'bodyguard' ).'
		...
		return 'wd_Walker_Nav_Menu_Edit';
