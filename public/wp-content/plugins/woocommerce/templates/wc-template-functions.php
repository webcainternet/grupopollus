...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_html__( 'New products', 'woocommerce' ) . '" href="' . esc_attr( $feed ) . '" />';
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'woocommerce' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'woocommerce' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_default_product_tabs' ) ) {
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( esc_html__( 'Reviews (%d)', 'woocommerce' ), get_comments_number( $post->ID ) ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_related_products' ) ) {
	...
	function woocommerce_related_products( $args = array(), $columns = false, $orderby = false ) {
		if ( ! is_array( $args ) ) {
			_deprecated_argument( __FUNCTION__, '2.1', esc_html__( 'Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.', 'woocommerce' ) );
...
if ( ! function_exists( 'woocommerce_order_again_button' ) ) {
	...
	function woocommerce_order_again_button( $order ) {
		...
		?>
		<p class="order-again">
			<a href="<?php echo wp_nonce_url( add_query_arg( 'order_again', $order->id ) , 'woocommerce-order_again' ); ?>" class="button"><?php esc_html_e( 'Order Again', 'woocommerce' ); ?></a>
		</p>
		<?php
	}
}
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		case "country" :
			...
			if ( sizeof( $countries ) == 1 ) {
				...
				$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_html__( 'Update country', 'woocommerce' ) . '" /></noscript>';
